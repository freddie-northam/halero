...
describe("readableError", () => {
    ...
    const zodMessage = JSON.stringify([
      {
        code: "custom",
        path: ["baseUrl"],
        message: "Base URL must be a full URL starting with http:// or https://.",
      },
    ]);
  ...
  test("joins multiple issue messages", () => {
    ...
    expect(readableError(new Error(zodMessage))).toBe("First problem. Second problem.");
  });

  test("passes plain readable messages through untouched", () => {
    expect(readableError(new Error("Incorrect password."))).toBe("Incorrect password.");
  });

  test("leaves bracket-leading but non-zod messages alone", () => {
    expect(readableError(new Error("[core] something broke"))).toBe("[core] something broke");
  ...
  test("falls back for empty and non-Error values", () => {
    ...
    expect(readableError(undefined)).toBe("Something went wrong. Please try again.");
  });
});
